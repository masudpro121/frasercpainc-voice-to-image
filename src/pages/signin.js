    ...
    <div>
      <form onSubmit={handleSignin} className=" text-slate-950 flex justify-center  ">
       <div className="w-full max-w-[450px] m-10 mt-20 rounded-md gap-2 flex flex-col [&>*]:rounded-3xl [&>input]:px-3 [&>input]:bg-zinc-400 text-white [&>input]:py-2 [&>input]:outline-none [&>input]:mb-2  p-7 bg-zinc-600">
        ...
        <input id="email" value={email} onChange={t=>setEmail(t.target.value)} type="email" className="placeholder:text-white" placeholder="Email" />
        <label className="text-white" htmlFor="pass">Password</label>
        <input id="pass" value={password} onChange={t=>setPassword(t.target.value)} type="password" className="placeholder:text-white" placeholder="Password" />
